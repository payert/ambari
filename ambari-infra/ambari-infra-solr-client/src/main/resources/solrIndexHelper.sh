...
function print_help() {
  cat << EOF
   ...
   upgrade-index command arguments:
     ...
     -v, --version                           Lucene version to upgrade (default: 6.6.2, available: 6.6.2, 7.7.2)
EOF
}
...
function upgrade_core() {
  ...
  local LUCENE_VERSION=${4:?"usage <lucene_index_version> e.g.: 7.7.2"}
...
function upgrade_index_tool() {
  ...
  : ${INDEX_VERSION:?"Please set the INDEX_VERSION variable! (6.6.2 or 7.7.2)"}
